...
public class MyHashMap<K, V> extends AbstractMap<K, V>
    ...
    private static final long serialVersionUID = 362498820763118865L;
    ...
    public void printNode(){
        ...
        for (int i = 0; i < table.length; i++) {
            if(table[i]!=null){
                MyHashMap.Node<K,V> node = table[i];
    ...
    final MyHashMap.Node<K, V> removeNode(int hash, Object key, Object value,
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if (node != null && (!matchValue || (v = node.value) == value ||
                ...
                if (node instanceof MyHashMap.TreeNode)
    ...
    final class KeySet extends AbstractSet<K> {
        ...
        public final Spliterator<K> spliterator() {
            return new MyHashMap.KeySpliterator<>(MyHashMap.this, 0, -1, 0, 0);
        }
    ...
    final class Values extends AbstractCollection<V> {
        ...
        public final Spliterator<V> spliterator() {
            return new MyHashMap.ValueSpliterator<>(MyHashMap.this, 0, -1, 0, 0);
        }
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        ...
        return (es = entrySet) == null ? (entrySet = new MyHashMap.EntrySet()) : es;
    }
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public final boolean contains(Object o) {
            ...
            Map.Entry<?, ?> e = (Map.Entry<?, ?>) o;
            Object key = e.getKey();
            MyHashMap.Node<K, V> candidate = getNode(hash(key), key);
        ...
        public final Spliterator<Map.Entry<K, V>> spliterator() {
            return new MyHashMap.EntrySpliterator<>(MyHashMap.this, 0, -1, 0, 0);
        }
    ...
    public V computeIfAbsent(K key,
        ...
        int n, i;
        int binCount = 0;
        MyHashMap.TreeNode<K, V> t = null;
        MyHashMap.Node<K, V> old = null;
        if (size > threshold || (tab = table) == null ||
                (n = tab.length) == 0)
            n = (tab = resize()).length;
        if ((first = tab[i = (n - 1) & hash]) != null) {
            if (first instanceof MyHashMap.TreeNode)
                old = (t = (MyHashMap.TreeNode<K, V>) first).getTreeNode(hash, key);
            else {
                MyHashMap.Node<K, V> e = first;
                K k;
                do {
                    if (e.hash == hash &&
                            ((k = e.key) == key || (key != null && key.equals(k)))) {
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (mappings < 0)
            ...
            threshold = ((cap < MAXIMUM_CAPACITY && ft < MAXIMUM_CAPACITY) ?
                    (int) ft : Integer.MAX_VALUE);
            @SuppressWarnings({"rawtypes", "unchecked"})
            MyHashMap.Node<K, V>[] tab = (MyHashMap.Node<K, V>[]) new MyHashMap.Node[cap];
            table = tab;
    ...
    abstract class HashIterator {
        MyHashMap.Node<K, V> next;        // next entry to return
        MyHashMap.Node<K, V> current;     // current entry
        int expectedModCount;  // for fast-fail
        int index;             // current slot

        HashIterator() {
            expectedModCount = modCount;
            MyHashMap.Node<K, V>[] t = table;
            current = next = null;
            index = 0;
        ...
        final MyHashMap.Node<K, V> nextNode() {
            MyHashMap.Node<K, V>[] t;
            MyHashMap.Node<K, V> e = next;
    ...
    final class KeyIterator extends MyHashMap.HashIterator
    ...
    static class HashMapSpliterator<K, V> {
        ...
        MyHashMap.Node<K, V> current;          // current node
        ...
        final int getFence() { // initialize fence and size on first use
            ...
            if ((hi = fence) < 0) {
                ...
                MyHashMap.Node<K, V>[] tab = m.table;
    ...
    static final class KeySpliterator<K, V>
            extends MyHashMap.HashMapSpliterator<K, V>
        ...
        public MyHashMap.KeySpliterator<K, V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyHashMap.KeySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
        ...
        public boolean tryAdvance(Consumer<? super K> action) {
            ...
            if (action == null)
                throw new NullPointerException();
            MyHashMap.Node<K, V>[] tab = map.table;
    ...
    static final class ValueSpliterator<K, V>
        ...
        public MyHashMap.ValueSpliterator<K, V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyHashMap.ValueSpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class EntrySpliterator<K, V>
        ...
        public MyHashMap.EntrySpliterator<K, V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyHashMap.EntrySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class TreeNode<K, V> extends MyHashMap.Entry<K, V> {
         ...
         */
        final MyHashMap.TreeNode<K, V> putTreeVal(MyHashMap<K, V> map, MyHashMap.Node<K, V>[] tab,
                                                  int h, K k, V v) {
            Class<?> kc = null;
